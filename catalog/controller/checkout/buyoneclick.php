	...
	public function submit() {
		...
		if ($product_info) {
            ...
            if (!$json) {
				...
				$this->load->model('checkout/bocorder');
				$data['order_id'] = $this->model_checkout_bocorder->addBocorder($order_data);
				if (empty($data['order_id'])) {
					$json['error']['order'] = $this->language->get('error_order');
				} else {
					$this->session->data['order_id'] = $data['order_id'];
					$this->model_checkout_bocorder->addBocorderHistory($data['order_id'], $this->config->get('config_order_status_id'), $exan);

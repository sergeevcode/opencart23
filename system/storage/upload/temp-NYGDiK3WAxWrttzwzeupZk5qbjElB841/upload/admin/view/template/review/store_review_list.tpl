...
<div id="content">
  ...
  <div class="container-fluid">
    <div id="sr_lite_notification" class="alert alert-info">
      <div id="sr_lite_loading"><i class="fa fa-spinner"> </i> <?php echo $text_load_message; ?></div>
    </div>
...
$('#button-filter').on('click', function() {
	url = 'index.php?route=review/store_review&token=<?php echo $token; ?>';
...
//--></script>
  <script type="text/javascript"><!--
    ...
    function getNotifications() {
      $('#sr_lite_loading').empty().html('<div id="sr_lite_loading"><i class="fa fa-spinner"> <?php echo $text_load_message; ?></div>');
      setTimeout(
              function(){
                $.ajax({
                  type: 'GET',
                  url: 'index.php?route=review/store_review/getNotifications&token=<?php echo $token; ?>',
                  dataType: 'json',
                  success: function(json) {
                    if (json['error']) {
                      $('#sr_lite_loading').empty().html(json['error']+' <span style="cursor:pointer;float:right;" onclick="getNotifications();"><i class="fa fa-refresh"></i> <?php echo $text_retry; ?></span>');
                    } else if (json['message']) {
                      $('#sr_lite_loading').html(json['message']);
                    } else {
                      $('#sr_lite_loading').html('<?php echo $error_no_message; ?>');
                    }
                  },
                  failure: function(){
                    $('#sr_lite_loading').html('<?php echo $error_message; ?> <span style="cursor:pointer;float:right;" onclick="getNotifications();"><i class="fa fa-refresh"></i> <?php echo $text_retry; ?></span>');
                  },
                  error: function() {
                    $('#sr_lite_loading').html('<?php echo $error_message; ?> <span style="cursor:pointer;float:right;" onclick="getNotifications();"><i class="fa fa-refresh"></i> <?php echo $text_retry; ?></span>');
                  }
